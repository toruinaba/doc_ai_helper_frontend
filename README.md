...
1. **左側パネル（RepositoryNavigator）の一時無効化**
   - ファイルツリー機能にバグがあるため、一時的に無効化
   - 将来の目次（Table of Contents）機能実装に備えて再利用可能な状態で保持
   - バックアップ版を `RepositoryNavigator.backup.vue` として保存
   - プレースホルダー表示で将来の機能をユーザーに説明

2. **将来実装予定: 目次（Table of Contents）機能**
   - 現在表示中のドキュメントの見出し構造を抽出
   - 階層的な目次表示機能
   - 見出しクリックによるスクロール機能
   - 現在の読書位置のハイライト表示
...
3. **将来実装予定: 目次ナビゲータ（Table of Contents）**
   - 現在表示中のドキュメントの見出し構造を抽出・表示
   - 階層的な目次表示とクリックナビゲーション
   - 現在の読書位置のハイライト表示
   - ファイルツリーから目次表示へのアプローチ変更

4. **将来実装予定: リポジトリナビゲータ（修正版）**
