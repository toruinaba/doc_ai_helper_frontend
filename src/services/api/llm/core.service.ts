...
export async function sendLLMQuery(
  ...
  if (systemPrompt && (!request.query.conversation_history || request.query.conversation_history.length === 0)) {
    ...
    mutableRequest.query = {
      ...request.query,
      conversation_history: createConversationWithSystemPrompt(systemPrompt)
    };
  } else if (systemPrompt) {
    const { createConversationWithSystemPrompt } = await import('./conversation.service');
    mutableRequest.query = {
      ...request.query,
      conversation_history: createConversationWithSystemPrompt(
        systemPrompt, 
  ...
  if (shouldUseMockApi()) {
    ...
    const mockResponse = getMockLLMResponse(mutableRequest.query.prompt, mutableRequest.query.conversation_history || []) as LLMResponse;
    
    // モックレスポンスに会話履歴が含まれていなければ作成する
    if (!mockResponse.optimized_conversation_history && mutableRequest.query.conversation_history) {
      // 最新のユーザーメッセージとシステムメッセージを保持
      const systemMessages = mutableRequest.query.conversation_history.filter(msg => msg.role === 'system');
      const userMessages = mutableRequest.query.conversation_history.filter(msg => msg.role === 'user');
  ...
  try {
    ...
    console.log('Sending LLM query with conversation history:', 
      mutableRequest.query.conversation_history ? mutableRequest.query.conversation_history.length : 0, 'messages');
    
    // デバッグ用：最初と最後のメッセージを表示
    if (mutableRequest.query.conversation_history && mutableRequest.query.conversation_history.length > 0) {
      console.log('First message:', mutableRequest.query.conversation_history[0]);
      console.log('Last message:', mutableRequest.query.conversation_history[mutableRequest.query.conversation_history.length - 1]);
    }
    
    const response = await apiClient.sendLLMQuery(mutableRequest);
